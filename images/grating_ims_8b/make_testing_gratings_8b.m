...
root_save = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_8b/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
for nn = 1:length(noise_levels)
    ...
    for ff = 1:length(my_freqs_cpd)
        ...
        thisdir = fullfile(root_save, sprintf('SF_%.2f_OrientSD_%.2f',my_freqs_cpd(ff), noise_levels(nn)));
        if ~isdir(thisdir)
            mkdir(thisdir)
        end
        ...
        for oo=1:length(orient_vals)
            ...
            rndseed = rndseed+1;
            images = get_filtered_noise_Version2(params,numInstances,rndseed);
