...
root_save = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_12a/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
for nn=1:length(noise_levels)
    ...
    for ff = 1:length(freq_levels_cpd)
        ...
        thisdir = sprintf('%sSF_%.2f_Noise_%.2f/', root_save, freq_levels_cpd(ff), noise_levels(nn));
        if ~isdir(thisdir)
            mkdir(thisdir)
        end
