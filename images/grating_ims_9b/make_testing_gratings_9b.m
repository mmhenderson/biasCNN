...
root_save = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_9b/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
% folder.
freq_levels_cpd = [1.21];

% specify different amounts of noise
noise_levels = [0.01];

% specify different contrast levels
contrast_levels = logspace(log10(0.01),log10(0.95),6);
...
for cc=1:length(contrast_levels)
    for ff = 1:length(freq_levels_cpd)
        ...
        thisdir = sprintf('%sSF_%.2f_Contrast_%.3f/', root_save, freq_levels_cpd(ff),contrast_levels(cc));
        if ~isdir(thisdir)
            mkdir(thisdir)
        end
        ...
        for oo=1:length(orient_vals)
            ...
            for pp=1:length(phase_levels)
                ...
                phase_vals = phase_levels(pp)*ones(numInstances, 1)*pi/180;
                ...
                for ii = 1:numInstances
                    ...
                    fn2save = fullfile(thisdir,sprintf('Gaussian_phase_%d_ex%d_%ddeg.png',phase_levels(pp),ii,orient_vals(oo)));
