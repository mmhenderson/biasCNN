...
root_save = '/usr/local/serenceslab/maggie/oriSpin_BEHAV/grating_figs/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
p.InnerDonutRadius = 1.5*p.ppd;
p.OuterFixRadius = 0.1*p.ppd; %outer dot radius (in pixels)
...
%                 fprintf('saving to %s\n', fn2save)
                ...
                %Give the grating the right contrast level and scale it
                stim = max(0,min(255,p.MyGrey+p.MyGrey*(p.Contrast* stim)))/255;

              
                % add a black circular fix pt
                centerPix = size(stim)./2+0.5;
                [xgrid, ygrid] = meshgrid(1:size(stim));
                xgrid = xgrid(:); ygrid = ygrid(:);
                inds2chg = sqrt((xgrid-centerPix(1)).^2 + (ygrid-centerPix(2)).^2) < p.OuterFixRadius;
                inds_lin = sub2ind(size(stim), xgrid(inds2chg), ygrid(inds2chg));
                stim(inds_lin) = 0;
                
                
                fn2save = [thisdir, 'Gaussian_randphase' num2str(pp) '_' sprintf('%d', p.TargOrient(tt)) 'deg.png'];
                imwrite(stim, fn2save)
                fprintf('saving to %s...\n', fn2save)
            end
        end
