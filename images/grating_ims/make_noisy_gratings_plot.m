% make a bunch of gratings at different orientations, create a figure to
% visualize the spatial frequency and noise variations
...
root_save = '/usr/local/serenceslab/maggie/biasCNN/grating_ims/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
% folder.
my_freqs_cpd = logspace(log10(.4), log10(2.2), 4);
...
for nn = 1:length(noise_levels)
    for ff = 1:length(my_freqs_cpd)
        ...
        thisdir = sprintf('%sSF_%.2f_noise%.2f/', root_save, my_freqs_cpd(ff), noise_levels(nn));
        if ~isdir(thisdir)
            mkdir(thisdir)
        end
...
%         for tt=1:length(p.TargOrient)
        tt = or2plot;
            p.PhaseJitter = randsample(0:179,numRandPhases)*(pi/180);

            for pp = 1:numRandPhases
...
%                 figure(1);hold all;colormap(gray)
%                 h1 = [h1, subplot(length(my_freqs_cpd), length(noise_levels), (ff-1)*length(noise_levels)+nn)];
%                 hold all;
%                 imagesc(image1);
%                 axis equal
%                 set(gca,'XTick',[], 'YTick',[],'XLim',[0.5, 140.5],'YLim',[0.5,140.5]);
% %                 
%                 
%                 if nn==1
%                     ylabel(sprintf('SF=%.2f cpp', my_freqs_cpp(ff)))
%                 end
%                 if ff==1
%                     title(sprintf('Noise=%.2f', noise_levels(nn)))
%                 end
...
%                 fprintf('saving to %s\n', fn2save)
                ...
                figure(2);hold all;colormap(gray)
                h2 = [h2, subplot(length(my_freqs_cpd), length(noise_levels), (ff-1)*length(noise_levels)+nn)];
                hold all;
                imagesc(image2);
                axis equal
                set(gca,'XTick',[], 'YTick',[],'XLim',[0.5, 140.5],'YLim',[0.5,140.5]);
...
% figure(1);hold all;
% set(gcf,'Color','w')
% suptitle(sprintf('Full-field gratings (%.0f degrees)', or2plot));

figure(2);hold all;
set(gcf,'Color','w')                
suptitle(sprintf('Gaussian windowed gratings (%.0f degrees)', or2plot));
