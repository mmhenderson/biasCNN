...
root_save = '/usr/local/serenceslab/maggie/biasCNN/grating_ims_7a/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
% how many phases do you want to make? 
numInstances = 48;
...
for nn = 1:length(noise_levels)
    for ff = 1:length(my_freqs_cpd)
        ...
        for tt=1:length(p.TargOrient)
            p.PhaseJitter = linspace(0,360,numInstances+1)*(pi/180);
            p.PhaseJitter(end) = [];
...
%                 fprintf('saving to %s\n', fn2save)
                ...
                fn2save = fullfile(thisdir, sprintf('Gaussian_phase%.2f_%ddeg.png', p.PhaseJitter(pp), p.TargOrient(tt)));
