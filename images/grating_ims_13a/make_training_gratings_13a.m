...
root_save = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_13a/';
if ~isdir(root_save)
    mkdir(root_save)
end
...
% folder.
freq_levels_cpd = logspace(log10(0.20),log10(4),6);
freq_levels_cpd = freq_levels_cpd(1);
        ...
        for oo=1:length(orient_vals)
            ...
            for pp=1:length(phase_levels)
                ...
                for ii = 1:numInstances
                    ...
                    fn2save = fullfile(thisdir,sprintf('Gaussian_contrast_%.2f_phase%d_ex%d_%ddeg.png',contrast_levels(cc),phase_levels(pp), ii,orient_vals(oo)));
