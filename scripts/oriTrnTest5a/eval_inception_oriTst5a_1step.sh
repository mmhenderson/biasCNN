...
load_log_dir=${root}logs/inception_oriTrn5a_short
...
save_eval_dir=${root}activations/inception_oriTst5a_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/inception_oriTst5a_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
...
cd ${codepath}
python reduce_weights_inception_oriTst5.py \
