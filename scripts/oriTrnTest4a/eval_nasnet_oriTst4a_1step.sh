...
load_log_dir=${root}logs/nasnet_oriTrn4a_short
...
save_eval_dir=${root}activations/nasnet_oriTst4a_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/nasnet_oriTst4a_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
...
cd ${codepath}
python reduce_weights_nasnet_oriTst4.py \
