...
load_log_dir=${root}logs/inception_oriTrn4a_short

dataset_name=oriTst4a
	
which_model=inception_v3
	
dataset_dir=${root}datasets/oriTst4a
if [ ! -d ${dataset_dir} ]
then
...
save_eval_dir=${root}activations/inception_oriTst4a_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/inception_oriTst4a_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
...
cd ${codepath}
python reduce_weights_inception_oriTst4.py \
