...
load_log_dir=${root}logs/nasnet_retrained_grating_orient_sf_long

dataset_name=grating_orient_sf_withnoise
	
which_model=nasnet_large
	
# these must match what is in the datasets folder
#declare -a noise_levels=(0.00 0.20 0.40 0.60 0.80)
declare -a noise_levels=(0.20 0.40 0.60 0.80)

for nn in ${noise_levels[@]}
do    

    dataset_dir=${root}datasets/testing/noise$nn
    if [ ! -d ${dataset_dir} ]
    then
    ...
    save_eval_dir=${root}activations/nasnet_activations_long_full/noise$nn
    if [ ! -d ${save_eval_dir} ]
    then
    ...
    reduced_dir=${root}activations/nasnet_activations_long_reduced/noise$nn
    if [ ! -d ${reduced_dir} ]
    then
    	mkdir -p ${reduced_dir}
    fi
    ...
    cd ${codepath}
    python reduce_weights_nasnet.py \
