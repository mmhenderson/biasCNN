...
load_log_dir=${root}logs/vgg16_oriTrn2_short

dataset_name=oriTst2
	
which_model=vgg_16
	
dataset_dir=${root}datasets/oriTst2
if [ ! -d ${dataset_dir} ]
then
...
save_eval_dir=${root}activations/vgg16_oriTst2_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/vgg16_oriTst2_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
