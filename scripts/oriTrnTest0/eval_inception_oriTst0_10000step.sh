...
load_log_dir=${root}logs/inception_retrained_grating_orient_sf_long
...
do    
    ...
    save_eval_dir=${root}activations/inception_activations_long_full/noise$nn
    if [ ! -d ${save_eval_dir} ]
    then
    ...
    reduced_dir=${root}activations/inception_activations_long_reduced/noise$nn
    if [ ! -d ${reduced_dir} ]
    then
    	mkdir -p ${reduced_dir}
    fi
