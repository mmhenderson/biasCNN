...
load_log_dir=${root}logs/inception_retrained_grating_orient_sf_short
...
do    
    ...
    save_eval_dir=${root}activations/inception_activations_short_full/noise$nn
    if [ ! -d ${save_eval_dir} ]
    then
    ...
    reduced_dir=${root}activations/inception_activations_short_reduced/noise$nn
    if [ ! -d ${reduced_dir} ]
    then
    	mkdir -p ${reduced_dir}
    fi
    ...
    cd ${codepath}
    python reduce_weights_inception.py \
