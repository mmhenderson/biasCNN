...
load_log_dir=${root}logs/inception_oriTrn1_short

dataset_name=oriTst1
	
which_model=inception_v3
	
dataset_dir=${root}datasets/oriTst1
if [ ! -d ${dataset_dir} ]
then
...
save_eval_dir=${root}activations/inception_oriTst1_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/inception_oriTst1_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
