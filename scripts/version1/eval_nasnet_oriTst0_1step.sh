...
load_log_dir=${root}logs/nasnet_retrained_grating_orient_sf_short
...
do    
    ...
    save_eval_dir=${root}activations/nasnet_activations_short_full/noise$nn
    if [ ! -d ${save_eval_dir} ]
    then
    ...
    reduced_dir=${root}activations/nasnet_activations_short_reduced/noise$nn
    if [ ! -d ${reduced_dir} ]
    then
    	mkdir -p ${reduced_dir}
    fi
