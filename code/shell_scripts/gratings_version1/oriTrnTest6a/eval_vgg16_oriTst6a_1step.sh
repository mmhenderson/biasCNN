...
load_log_dir=${root}logs/vgg16_oriTrn6a_short

dataset_name=oriTst6a
	
which_model=vgg_16
	
dataset_dir=${root}datasets/oriTst6a
if [ ! -d ${dataset_dir} ]
then
...
save_eval_dir=${root}activations/vgg16_oriTst6a_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/vgg16_oriTst6a_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
...
cd ${codepath}
python reduce_weights_vgg16_oriTst6a.py \
