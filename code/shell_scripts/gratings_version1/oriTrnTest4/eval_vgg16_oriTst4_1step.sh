...
load_log_dir=${root}logs/vgg16_oriTrn4_short

dataset_name=oriTst4
	
which_model=vgg_16
	
dataset_dir=${root}datasets/oriTst4
if [ ! -d ${dataset_dir} ]
then
...
save_eval_dir=${root}activations/vgg16_oriTst4_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/vgg16_oriTst4_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi
