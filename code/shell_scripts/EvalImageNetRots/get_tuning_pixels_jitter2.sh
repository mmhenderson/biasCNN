...
rand_seed_jitter=239478

# am i over-writing old folders, or checking which exist already?
overwrite=0
TEST=0

dataset_root=FiltIms12AllSFCos
which_model=pixel_relu
...
do
	...
	${ROOT}biasCNN/code/shell_scripts/EvalImageNetRots/get_tuning_single_pixels.sh ${dataset_name} ${ROOT} ${overwrite} ${TEST} ${which_model}
