...
load_log_dir=${root}checkpoints/vgg16_ckpt/vgg_16.ckpt

# where we save the large complete activation files
save_eval_dir=${root}activations/vgg16/pretrained_vgg16_${dataset_name}_short_full
if [ ! -d ${save_eval_dir} ]
then
...
reduced_dir=${root}activations/vgg16/pretrained_vgg16_${dataset_name}_short_reduced
if [ ! -d ${reduced_dir} ]
then
	mkdir -p ${reduced_dir}
fi    
