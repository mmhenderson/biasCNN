...
do
    ...
    load_log_dir=${root}logs/vgg16/ImageNet/scratch_vgg16_imagenet_rot_${rot}/weightdecay_0.00005_rmspropdecay_0.90_rmspropmomentum_0.80_learningrate_0.005_learningratedecay_0.94/model.ckpt-${step_num}

    # where we save the large complete activation files
    save_eval_dir=${root}activations/vgg16/scratch_vgg16_imagenet_rot_${rot}_${dataset_name}_short_full
    if [ ! -d ${save_eval_dir} ]
    then
    ...
    reduced_dir=${root}activations/vgg16/scratch_vgg16_imagenet_rot_${rot}_${dataset_name}_short_reduced
    if [ ! -d ${reduced_dir} ]
    then
    	mkdir -p ${reduced_dir}
    fi    
