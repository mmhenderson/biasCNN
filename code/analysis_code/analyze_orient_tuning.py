...
def von_mises_deg180(xx,mu,k,a,b):
  ...
  xx_rad2pi = np.float128(xx/180*2*np.pi)
...
def analyze_orient_tuning(root, model, training_str, dataset_all, nSamples, param_str, ckpt_str):
  ...
  for ll in range(nLayers):
    ...
    files_this_layer = [ff for ff in files_done if layer_labels[ll] in ff]
    n_done = np.size(files_this_layer)
    if n_done>=5:
      continue
    ...
    for sf in range(nSF):
      ...
      for uu in units2do:
        ...
        try:
          ...
          params, params_covar = scipy.optimize.curve_fit(von_mises_deg180, ori_axis, real_y, p0=params_start, bounds=((-0.0001,10**(-15),-np.inf,-np.inf),(180-0.0001,np.inf,np.inf,np.inf)))
        except:
    ...
    save_name =os.path.join(save_path,'%s_fit_r2_each_sample_eval_at_ckpt_%s0000.npy'%(layer_labels[ll], ckpt_str[0:2]))
    print('saving to %s\n'%save_name)
