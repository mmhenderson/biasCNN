...
for tr in range(nTrainingSchemes):
  ...
  for ii in range(nInits):
    ...
    for kk in range(nImageSets):
      ...
      ckpt_dirs = os.listdir(os.path.join(root,'code','mahal_dist_pairwise',model,training_str,param_str,dataset))
      ...
      save_path = os.path.join(root,'code','mahal_dist_pairwise',model,training_str,param_str,dataset,'eval_at_ckpt-%s_reduced_varyncomps'%nums[0],'Mahal_dist_all_pairwise.npy')
      ...
      if kk==0 and tr==0 and ii==0:
        ...
        nOri = np.shape(vals)[2]      
        # initialize this ND array to store all Fisher info calculated values
        all_mahal_dist = np.zeros([nTrainingSchemes, np.max(nInits_list), nImageSets, nLayers, nSF, nOri, nOri])
       
      all_mahal_dist[tr,ii,kk,:,sf,:,:] = np.squeeze(vals);
#%% create color map
nsteps = 8
...
for ll in range(np.size(layers2plot)):
  ...
  for ii in init2plot:
    ...
    acc_mat = np.mean(all_mahal_dist[tr,ii,:,layers2plot[ll],sf,:,:],axis=0)
...
plt.suptitle('%s\nMahalanobis distance between pairs of orients, using top %d PCs' % (training_strs[tr], ncomp2use))
...
for ww1 in range(np.size(layers2plot)):
  ...
  for pp in range(len(pp2plot)):
    ...
    for kk in range(nImageSets):
      ...
      # FI is nOri pts long
      # average over image sets
      acc_mat = np.mean(all_mahal_dist[tr,ii,:,layers2plot[ww1],sf,:,:],axis=0)
      #get off-diagonal
