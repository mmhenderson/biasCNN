...
#              allw = np.concatenate((allw, w[:,0:3]), axis=0)
      ...
      #%% identify all units with constant signal for all stimuli (these will mess up Fisher information calculation)      
      constant_inds = np.all(np.equal(allw, np.tile(np.expand_dims(allw[0,:], axis=0), [np.shape(allw)[0], 1])),axis=0)
      # take out all the constant units, leaving only units with variance over images
