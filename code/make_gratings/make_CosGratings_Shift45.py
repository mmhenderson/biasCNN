...
This version will have the labels shifted 45 away from the actual orientations
anisotropy should shift. make sure there is nothing buggy in the code!
MMH 1/27/20
...
for ss in range(nSets):
  ...
  if ss==0:
    image_folder = os.path.join(root, 'biasCNN','images','gratings','CosGratings_Shift45')
  else:
    image_folder = os.path.join(root, 'biasCNN','images','gratings','CosGratings_Shift45' + np.str(ss))
          ...
          for oo in range(np.size(orient_vals)):
              ...
              for pp in range(np.size(phase_levels)):
                  ...
                  for ii in range(numInstances):
                      ...
                      sine = (np.sin(this_freq_cpp*2*np.pi*(y*np.sin((orient_vals[oo]+shift_by)*np.pi/180)+x*np.cos((orient_vals[oo]+shift_by)*np.pi/180))-phase_vals[ii]));
