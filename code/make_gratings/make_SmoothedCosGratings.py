...
nSets = 4
seeds=[923298,126872,912982,328377]

image_set_name = 'SmoothedCosGratings'
...
# smooth it
cos_mask_smoothed = scipy.ndimage.filters.gaussian_filter(cos_mask,sigma=Smooth_sd)

#plt.figure();
#plt.pcolormesh(cos_mask_smoothed)
#plt.axis('square')
#plt.figure();
#plt.plot(range(image_size),cos_mask[100,:])
#plt.plot(range(image_size),cos_mask_smoothed[100,:])

# make it three color channels
mask_image = np.tile(np.expand_dims(cos_mask_smoothed,2),[1,1,3])
...
for ss in range(nSets):
  ...
  if ss==0:
    image_folder = os.path.join(root, 'biasCNN','images','gratings',image_set_name)
  else:
    image_folder = os.path.join(root, 'biasCNN','images','gratings',image_set_name + np.str(ss))
