...
nSetsToMake = 4;
randseeds = [123235,603755,129884,498585];
sets2make_now = [1:4];
...
image_set_root = 'FiltIms10AllSFCos';
...
% C must be >=360
min_sf_ind = 360/(2*pi);
min_sf = fax(center_pix+ceil(min_sf_ind));

freq_sd_cpp=0.01;
freq_min_cpp = min_sf + freq_sd_cpp;
% freq_min_cpp = freq_step_cpp + freq_sd_cpp;
freq_max_cpp = nyq - freq_sd_cpp;

% 
tar_mag = normcdf(mag_grid, freq_min_cpp, freq_sd_cpp).*1-normcdf(mag_grid, freq_max_cpp,freq_sd_cpp);

% figure;imagesc(tar_mag);colorbar()
...
%     rand_sets = reshape(rand_sets,nOri, nImsPerOri);
    ...
    ori_filts = zeros(final_size_pix,final_size_pix,nOri);
    for oo = 1:nOri
...
%             figure;imagesc(image_fake);colorbar();
%             
%             
%             [out] = fft2(image_masked);
%             image_fft = fftshift(out);  
%             mag_orig = abs(image_fft);
%             phase_orig = angle(image_fft);
...
%             image_fft_filt_shuff = complex(mag.*cos(phase_shuff), mag.*sin(phase_shuff));
%             
%             % bcck to spatial domain
%             image_filtered = real(ifft2(fftshift(image_fft_filt_shuff)));  
%             
%             figure;imagesc(image_filtered);colorbar();
