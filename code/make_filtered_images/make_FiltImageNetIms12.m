...
nSetsToMake = 4;
randseeds = [834874,129387,389478,382784];
sets2make_now = [1:4];
...
image_set_root = 'FiltIms12AllSFCos';
...
%%  make one filter now, just to figure out how big they need to be

GaborBank = gabor(1./freq_mean_cpp,180-orient_vals_deg(1), 'SpatialFrequencyBandwidth',sf_bw_cpp,'SpatialAspectRatio',ori_bw_rad/sf_bw_cpp);
...
for ss = [sets2make_now]
    ...
    for oo = 1:nOri
        ...
        GaborBank = gabor(1./freq_mean_cpp,180-orient_vals_deg(oo), 'SpatialFrequencyBandwidth',sf_bw_cpp,'SpatialAspectRatio',ori_bw_rad/sf_bw_cpp);       
        assert(all(sizeLargestKernel == size(GaborBank(1).SpatialKernel)));
        
        my_filter_freq = makeFrequencyDomainTransferFunction_MMH(GaborBank(1),size_after_pad);        
