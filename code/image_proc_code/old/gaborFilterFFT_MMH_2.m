function [M,P] = gaborFilterFFT_MMH(A,GaborBank,displayWaitBar)
...
% A = padarray(A,padSize,'symmetric');
sizeAPadded = size(A);
...
for p = 1:length(GaborBank)
    ...
    H = makeFrequencyDomainTransferFunction_MMH(GaborBank(p),sizeAPadded,class(A));
