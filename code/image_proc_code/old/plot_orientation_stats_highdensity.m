...
for ss=1:nSets
    ...
    if ss<=nSets1     
        ...
        fn2load = fullfile(root, 'ImageStats', sprintf('synset%d_allstats_highdensity.mat',ss));
    elseif ss<=nSets1+nSets2
        fprintf('loading from %s\n',extra_synset_folders{ss-nSets1});
        fn2load = fullfile(root, 'ImageStats', sprintf('synset_%s_allstats_highdensity.mat',extra_synset_folders{ss-nSets1}));
    else
        fprintf('loading from %s\n',extra_folders(ss-nSets1-nSets2).name);
        fn2load = fullfile(root, 'ImageStats', sprintf('%s_allstats_highdensity.mat',extra_folders(ss-nSets1-nSets2).name));
    end
