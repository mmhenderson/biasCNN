...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst11/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_11/'
  
dataset_name = 'oriTst11'
# information about the stimuli. 
sf_vals = np.logspace(np.log10(0.2),np.log10(4),6)
stim_types = ['Gaussian']
nOri=180
nSF=6
nPhase=2;
...
noiselist = np.expand_dims(np.repeat(noise_levels, nPhase*nOri*nSF*nType*nEx),1)
exlist = np.transpose(np.tile(np.repeat(np.arange(nEx), nPhase*nOri*nSF*nType), [1,nNoiseLevels]))
typelist = np.transpose(np.tile(np.repeat(np.arange(nType), nPhase*nOri*nSF), [1,nNoiseLevels*nEx]))
orilist=np.transpose(np.tile(np.repeat(np.arange(nOri),nSF*nPhase), [1,nType*nNoiseLevels*nEx]))
sflist=np.transpose(np.tile(np.repeat(np.arange(nSF),nPhase),[1,nOri*nType*nNoiseLevels*nEx]))
phaselist=np.transpose(np.tile(np.arange(nPhase),[1,nOri*nSF*nType*nNoiseLevels*nEx]))

featureMat = np.concatenate((noiselist,exlist,typelist,orilist,sflist,phaselist),axis=1)
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_Contrast_%.2f" % (sf_vals[int(sflist[ii])], noiselist[ii])
