...
noiselist = np.expand_dims(np.repeat([1], nPhase*nOri*nSF*nType*nEx),1)
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "AllIms" % (sf_vals[int(sflist[ii])])
    filename = "FiltNoise_ex%d_%ddeg.png" % (exlist[ii]+1, orilist[ii])
...
def main(argv):
  ...
  for sf in range(len(sf_vals)):
    ...
    dataset_name = '%s_SF_%.2f'%(dataset_root,sf_vals[sf])
