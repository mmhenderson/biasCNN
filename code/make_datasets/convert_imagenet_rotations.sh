...
declare -a rot_list=(45 22)

for rot in ${rot_list[@]}
do   

    # Where the training/testing sets of images are now
    TRAIN_DIRECTORY="${IMAGE_DIR}train_rot_${rot}/"
    VALIDATION_DIRECTORY="${IMAGE_DIR}validation_rot_${rot}/"
    
    # where i'll put all the tfrecord files
    OUTPUT_DIRECTORY="${DATASET_DIR}tfrecord_rot_${rot}/"
