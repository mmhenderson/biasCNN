...
dataset_dir = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTrn13d/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_13d/'
  
dataset_name = 'oriTrn13d'
...
contrastlist = np.expand_dims(np.repeat(contrast_levels, nPhase*nOri*nSF), 1)
orilist=np.transpose(np.tile(np.repeat(np.arange(nOri),nSF*nPhase), [1,nContrastLevels]))
sflist=np.transpose(np.tile(np.repeat(np.arange(nSF),nPhase),[1,nOri*nContrastLevels]))
phaselist=np.transpose(np.tile(np.arange(nPhase),[1,nOri*nSF*nContrastLevels]))

featureMat = np.concatenate((contrastlist,orilist,sflist,phaselist),axis=1)
...
def _get_filenames_and_classes(image_dir):
  ...
  for ii in np.arange(0,np.size(model_labels)):
    ...
    filename = "Gaussian_contrast_%.2f_phase%d_ex1_%ddeg.png" % (contrastlist[ii], phase_vals[phaselist[ii,0]], orilist[ii])
