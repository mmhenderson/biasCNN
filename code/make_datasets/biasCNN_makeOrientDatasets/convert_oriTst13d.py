...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst13d/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_13d/'
  
dataset_name = 'oriTst13d'
...
nNoiseLevels = 1
noise_levels = [0.01]
contrast_levels  = np.logspace(np.log10(0.01), np.log10(0.95), 6)
nContrastLevels = 6
nEx = 4

# list all the image features in a big matrix, where every row is unique.
contrastlist = np.expand_dims(np.repeat(contrast_levels, nPhase*nOri*nSF*nType*nEx),1)
exlist = np.transpose(np.tile(np.repeat(np.arange(nEx), nPhase*nOri*nSF*nType), [1,nContrastLevels]))
typelist = np.transpose(np.tile(np.repeat(np.arange(nType), nPhase*nOri*nSF), [1,nContrastLevels*nEx]))
orilist=np.transpose(np.tile(np.repeat(np.arange(nOri),nSF*nPhase), [1,nType*nContrastLevels*nEx]))
sflist=np.transpose(np.tile(np.repeat(np.arange(nSF),nPhase),[1,nOri*nType*nContrastLevels*nEx]))
phaselist=np.transpose(np.tile(np.arange(nPhase),[1,nOri*nSF*nType*nContrastLevels*nEx]))

featureMat = np.concatenate((contrastlist,exlist,typelist,orilist,sflist,phaselist),axis=1)
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_Contrast_%.2f" % (sf_vals[int(sflist[ii])], contrastlist[ii])
    filename = "Gaussian_phase%d_ex%d_%ddeg.png" % (phase_vals[phaselist[ii,0]],exlist[ii]+1, orilist[ii])
