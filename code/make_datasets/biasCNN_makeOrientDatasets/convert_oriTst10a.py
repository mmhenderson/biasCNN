...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst10a/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_10a/'
  
dataset_name = 'oriTst10a'
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    ...
    filename = "Gaussian_fixedphase_%d_%ddeg.png" % (phaselist[ii]+1, orilist[ii])
