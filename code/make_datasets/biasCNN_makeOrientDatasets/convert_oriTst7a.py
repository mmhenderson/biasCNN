...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst7a/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/grating_ims_7a/'
  
dataset_name = 'oriTst7a'
...
nSF=1
nPhase=48
phase_levels = np.linspace(0,360,nPhase+1)
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    ...
    filename = "%s_phase%.2f_%ddeg.png" % (stim_types[int(typelist[ii])], phase_levels[phaselist[ii]], orilist[ii])
