...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst9b/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_9b/'
  
dataset_name = 'oriTst9b'
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_Contrast_%.3f" % (sf_vals[int(sflist[ii])], contrastlist[ii])
    filename = "Gaussian_phase_%d_ex%d_%ddeg.png" % (phase_vals[phaselist[ii,0]],exlist[ii]+1, orilist[ii])
