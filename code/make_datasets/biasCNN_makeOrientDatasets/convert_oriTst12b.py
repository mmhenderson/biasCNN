...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst12b/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_12b/'
  
dataset_name = 'oriTst12b'
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_Noise_%.2f" % (sf_vals[int(sflist[ii])], noiselist[ii])
