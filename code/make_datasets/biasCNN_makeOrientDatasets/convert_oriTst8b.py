...
dataset_dir_root = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTst8b/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/images/grating_ims_8b/'
  
dataset_name = 'oriTst8b'
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "SF_%.2f_OrientSD_%.2f" % (sf_vals[int(sflist[ii])], noiselist[ii])
