...
dataset_dir = '/usr/local/serenceslab/maggie/biasCNN/datasets/oriTrn6a/'
image_dir = '/usr/local/serenceslab/maggie/biasCNN/grating_ims_6a/'
  
dataset_name = 'oriTrn6a'
...
def _get_filenames_and_classes(image_dir):
  ...
  for ii in np.arange(0,np.size(model_labels)):
    ...
    filename = "%s_fixedphase_%d_%ddeg.png" % (stim_types[int(typelist[ii])], phaselist[ii]+1, orilist[ii])
