...
sf_vals = np.logspace(np.log10(0.0125),np.log10(0.250),nSF)
...
phase_complement_vals = [0,1]
nOri=180
# list all the image features in a big matrix, where every row is unique.
orilist=np.expand_dims(np.transpose(np.repeat(np.arange(nOri),nPhasePairs*nPhasePerPair)),axis=1)
phaselist = np.transpose(np.tile(np.repeat(np.arange(nPhasePairs), nPhasePerPair),[1,nOri]))
phase_complement_list=np.transpose(np.tile(np.arange(nPhasePerPair),[1,nOri*nPhasePairs]))

featureMat = np.concatenate((orilist,phaselist,phase_complement_list),axis=1)
...
#  for nn in np.arange(nNoiseLevels):
  for ii in np.arange(0,np.size(model_labels)):
    subfolder = "Phase_%.2f"%(phase_values[int(phaselist[ii,0])])
    filename = "CosineWindow_PhaseComplement%d_%ddeg.png" % (phase_complement_list[ii,0], orilist[int(ii)])
...
def main(argv):
  ...
  for sf in range(nSF):
    ...
    dataset_name = '%s_SF_%0.2f'%(dataset_root,sf_vals[sf])
