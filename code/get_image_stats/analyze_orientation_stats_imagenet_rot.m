...
%         mean_mag(ss,:,:) = mean(all_mag,3);   
        ...
        % convert to probability estimate, by dividing by the sum here.
        % magnitude(this orient)/magnitude(all orients)= P(this orient)
        ims_by_ori = ims_by_ori./repmat(sum(ims_by_ori,2),1,size(ims_by_ori,2));
%         ims_by_ori = zscore(ims_by_ori,[],2);
        ...
        % convert to probability estimate, by dividing by the sum here.
        % magnitude(this orient)/magnitude(all orients)= P(this orient)
        ims_by_ori_sf = ims_by_ori_sf./repmat(sum(ims_by_ori_sf,2),1,size(ims_by_ori_sf,2),1);
