...
% MMH 11/19/2019
    ...
    wavelist_bank = repmat(params.wavelength_list, 1, numel(params.ori_list));
    for sf = 1:length(params.wavelength_list)
        
        inds = wavelist_bank==params.wavelength_list(sf);
        vals = mag(:,:,inds);
        mag_norm(:,:,inds) = vals./repmat(sum(vals,3),1,1,size(vals,3));
        
    end
    
    mag_by_ori1 = squeeze(mean(mean(mag,2),1));
    mag_by_ori2 = squeeze(mean(mean(mag_norm,2),1));
    
    sf=3;
    figure;hold all;
    inds = wavelist_bank==params.wavelength_list(sf);
%     plot(zscore(mag_by_ori1(inds)));
%     plot(zscore(mag_by_ori2(inds
    plot(mag_by_ori1(inds)/sum(mag_by_ori1(inds)));
    plot(mag_by_ori2(inds));
