...
#%% run the IEM - within each sf separately
        ...
        for nn in noiselevels2plot:
            ...
            for sf in range(nSF):
                 for tt in range(nType):
                    ...
                    noiselist_now = noiselist[inds]
                    orilabs_now = ori_labs[inds]
                    
                    chan_resp_all = IEM.run_crossval_IEM_specify_labels(alldat,orilabs_now,phaselist_now)
                    
                    average_recons = IEM.shift_and_average(chan_resp_all,orilabs_now,center_deg);
...
for nn in noiselevels2plot:
    ...
    for ww1 in layers2plot:
        ...
        phaselist_now = phaselist[myinds]

        un,whichCV = np.unique(phaselist_now, return_inverse=True, axis=0)
        
        chan_resp_all = IEM.run_crossval_IEM_specify_labels(alldat, orilabs_now, whichCV)      
        average_recons = IEM.shift_and_average(chan_resp_all,orilabs_now,center_deg);
