...
% deg,  by loading random ImageNet images and filtering with a Gabor
% filter.
% Each image has narrow-band spatial frequency content, at six different
% levels between 0.01 cycles/pixel and 0.25 cycles/pixel.
% For each of these 6 spatial frequenies, will create 4 unique random 
% versions of this image set, each with 180*48 = 8640 images.
...
nSetsToMake = 4;
randseeds = [123355,293943,928343,129838];
sets2make_now = [1:4];
...
image_set_root = 'FiltIms13Cos';
% path where all the ImageNet ims are located
...
% define image orientations to make
...
%% Define spatial frequencies to create 
% these will each be in a separate folder. Units are cycles per pixel.
...
GaborBank = gabor(1./freq_levels_cpp,180-orient_vals_deg);
