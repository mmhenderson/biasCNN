# -*- coding: utf-8 -*-
"""
Created on Fri Mar  8 10:11:53 2019

@author: https://github.com/marionmari/pyGPs/blob/master/pyGPs/Optimization/minimize.py

Note: johnserences modified a bit to make work with fun_minLL_norm a bit more transparently
"""

from __future__ import division
from __future__ import print_function
from past.utils import old_div
from numpy import dot, squeeze, isinf, isnan, any, sqrt, isreal, real, nan, inf, finfo, mod
import logging
import sys
# import fun_minLL_norm
...
def minimize(f, X, W, noise, length=None, red=1.0, verbose=False):
    '''
    ...
    Translated by: https://github.com/marionmari/pyGPs/blob/master/pyGPs/Optimization/minimize.py
    '''
        ...
        while 1:                       # keep extrapolating as long as necessary
            ...
            while (not success) and (M > 0):
                try:
                    ...
                    result3 = f(X+x3*s, W, noise)
                    f3 = result3[0]
                    df3 = result3[1]
                    if isnan(f3) or isinf(f3) or any(isnan(df3)) or any (isinf(df3)):                        
                        raise TypeError('function gave bad value on M=%d, trying again' % M)
                    success = 1
...
#             print('x3', x3.shape)
            tmp = X+x3*s
#             print('tmp', tmp.shape)
            result3 = f(tmp.T, W, noise)
#            if any(~isreal([result3[0]])):
#                print(result3[0])
#                raise TypeError('function gave imaginary output')
            f3 = result3[0]
            df3 = result3[1]
            if f3 < F0:
                X0 = X+x3*s; F0 = f3; dF0 = df3               # keep best values
...
#        if mod(i,20)==0:
#            print('iteration %d: fX=%d' % (i,fX[-1]), end='\r')
#            sys.stdout.flush()
    if verbose:
