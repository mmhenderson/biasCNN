...
@author: johnserences, based on Ruben van Bergen's matlab code from 
Van Bergen, R. S., Ma, W.J., Pratte, M.S. & Jehee, J.F.M. (2015). 
Sensory uncertainty decoded from visual cortex predicts behavior. Nature
Neuroscience.

Most docstring info is from Ruben's code unless otherwise noted
"""

# imports

import numpy as np
from gen_fun import make_basis_fucntion, cosd
...
def gen_test_data(p, nvox = 100, ntrials = 288, nruns = 8, nchans = 8, sig_sim = 0.3, rho_sim = 0.05, span=180):
    ...
    s_sim = np.random.rand(ntrials) * 2 * np.pi
