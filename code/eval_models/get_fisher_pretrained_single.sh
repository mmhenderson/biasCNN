...
reduced_dir=${ROOT}biasCNN/code/fisher_info/${model_short}/pretrained/params1/${dataset_name}/eval_at_ckpt-0_full
...
then
	...
	then
		num_batches=96
		python get_fisher_info_full.py ${save_eval_dir} ${reduced_dir} ${model_short} ${dataset_name} ${num_batches}
...
reduced_files=($(ls ${reduced_dir}/*.npy))
num_reduced_files=${#reduced_files[@]}
echo "	there are $num_reduced_files files in fisher info folder"
if (( $num_reduced_files < 3 ))
then
	reallyDone=0
else
