...
save_dir=${ROOT}/code/tsep_new/${model_short}/scratch_imagenet_rot_${rot}/${which_hyperpars}/${dataset_name}/eval_at_ckpt-${step_num}_reduced_varyncomps
...
then
	...
	if [[ $TEST != 1 ]]
	then		
		python get_tsep_new_vary_ncomps.py ${reduced_dir} ${save_dir} ${model_short} ${dataset_name}
