...
# where we save the results of analysis
save_dir=${ROOT}/saved_analyses/tsep/${model_short}/pretrained/params1/${dataset_name}/eval_at_ckpt-0_reduced_varyncomps
...
then
	...
	if [[ $TEST != 1 ]]
	then		
		python get_tsep_vary_ncomps.py ${reduced_dir} ${save_dir} ${model_short} ${dataset_name}
